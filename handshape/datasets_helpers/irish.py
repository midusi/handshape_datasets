...
def preprocess_image(image, pad, image_size):
    ...
    image = transform.resize(
        image, (image_size[0], image_size[1]), preserve_range=True)
...
def load_data(test_subjects=[6], folderpath=os.path.join(expanduser("~"), ".keras",
    ...
    if not os.path.exists(np_filepath):
        print("Could not find ", np_filename,
              ". Downloading/extracting/reencoding dataset...")
    ...
    x_train, x_test, y_train, y_test, subject_train, subject_test = split_data(
